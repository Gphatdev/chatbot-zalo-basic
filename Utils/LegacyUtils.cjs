...
const logger = require("./LegacyLogger.cjs");
...
// zca-js >= 2.0.0 đã bỏ sharp khỏi thư viện lõi, nên phải tự cung cấp hàm lấy
// metadata (width/height/size) khi muốn gửi ảnh/gif bằng đường dẫn file, nếu
// không thì mọi lệnh api.sendMessage({ attachments: [...] }) gửi ảnh sẽ lỗi.
// Dùng "image-size" (đọc header, không decode ảnh) thay vì "sharp" để khỏi
// phải cài thêm dependency native.
